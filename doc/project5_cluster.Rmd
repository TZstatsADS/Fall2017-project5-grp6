...
The overall matching rate is quite low. Also we see that most participants were on their mid-20's, for this dataset.
...
We have also the data on whether people had the same race (yes or no) and their ratings on same race preference and same religion preference, been 1 not important for the other person to have the same race and 10 very important that the other person is of the same race.
...
If we look at the matching rates, having the same race or not does seems to have little impact on the rate of matches. We get 17.06% matching rate if individuals have the same race and 16.08%% matching rate if they have different races.
...
And we can observe the matching rates distribution for each individual, below too:
```{r}
...
However, as mentioned, the linear model is not the best fit for our matching rates. We can see that taking a quick look at the residuals:
...
### After testing these linear models, our conclusion was that: if you want to increasing your matching rates, you better start to like going out more often, clubbing and arts. Also, have more practice with dates. Which makes total sense, since people with these characteristics are generally more extrovert, less shy and tend to feel more confortable among people they don't know very well.
